...
from core.bench.models import BenchmarkConfig
...
def validate_benchmark_config(path: Path) -> BenchmarkConfig:
    ...
    if "task" not in cfg:
        raise PipelineError(
            f"Benchmark config requires a 'task' field: {path}",
            hint="Set task: detect."
        )
    
    model_task = str(cfg["task"]).strip().lower()
    if model_task not in ("detect", "segment", "classify", "pose", "obb"):
        raise PipelineError(
            f"Unsupported model task: {model_task}",
            hint="Choose one from list: 'detect', 'segment', 'classify', 'pose', 'obb'.",
        )
    
    if model_task != "detect":
        raise PipelineError(
            f"Unsupported task for bench report: {model_task}",
            hint=(
                "The current bench implementation supports only detection quality metrics. "
                "Set task: detect."
            )
        )
    ...
    dataset_cfg["quality"] = _merge_dataset_source(
        ...
        reset_yaml_split_to='val' if has_quality_override else None,
    )
    ...
    if int(cfg["quality"].get("max_images", 0) or 0) < 0:
        raise PipelineError("quality.max_images must be >= 0")
    
    try:
        int(cfg["quality"].get("seed", 42))
    except (TypeError, ValueError) as exc:
        raise PipelineError("quality.seed must be an integer") from exc
