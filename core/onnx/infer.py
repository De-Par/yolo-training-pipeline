...
def collect_input_images(source: Path) -> list[Path]:
    ...
    if source.is_dir():
        ...
        return images
    raise PipelineError(f'Input source does not exist: {source}', hint='Point --source to an image file or a directory with images.')
...
def _resolve_input_hw(session: Any, override_hw: tuple[int, int] | None) -> tuple[tuple[int, int] | None, bool]:
    ...
    if len(shape) != 4:
        raise PipelineError(f'Unsupported ONNX input rank: {shape}', hint='This demo expects a detection model with NCHW input.')
    h_raw = shape[2]
    w_raw = shape[3]
...
def _pick_detection_output(outputs: Sequence[np.ndarray]) -> np.ndarray:
    ...
    if not candidates:
        raise PipelineError('Could not find a 2D/3D detection output in ONNX results.', hint='This demo expects a detect ONNX model exported from the current YOLO pipeline.')
    arr = candidates[0]
    ...
    if arr.shape[0] <= 256 and arr.shape[1] > arr.shape[0]:
        arr = arr.T
    if arr.shape[1] < 6:
        raise PipelineError(f'Unsupported detection output shape: {arr.shape}', hint='Expected rows shaped as [x, y, w, h, class_scores...].')
    return arr.astype(np.float32, copy=False)


def _decode_predictions(raw: np.ndarray, class_names: Sequence[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    xywh = raw[:, :4]
    remaining = raw[:, 4:]
    if len(class_names) > 0 and remaining.shape[1] == len(class_names) + 1:
    ...
    return xywh, scores, class_ids.astype(np.int32, copy=False)
...
def _run_classwise_nms(boxes_xyxy: np.ndarray, scores: np.ndarray, class_ids: np.ndarray, *, conf: float, iou: float, max_det: int) -> list[int]:
    keep: list[int] = []
    for class_id in sorted(set(int(value) for value in class_ids.tolist())):
        ...
        nms_boxes = [[float(x1), float(y1), float(max(0.0, x2 - x1)), float(max(0.0, y2 - y1))] for x1, y1, x2, y2 in class_boxes]
...
def infer_image(
    ...
    *,
    ...
    xywh, scores, class_ids = _decode_predictions(raw, detector.class_names)
    boxes_xyxy = _xywh_to_xyxy(xywh)
    boxes_xyxy = _scale_boxes(boxes_xyxy, scale=scale, pad=pad, orig_hw=(orig_h, orig_w))
    keep = _run_classwise_nms(boxes_xyxy, scores, class_ids, conf=conf, iou=iou, max_det=max_det)
...
def draw_detections(result: InferenceResult, image_bgr: np.ndarray) -> np.ndarray:
    ...
    for det in result.detections:
        ...
        (text_w, text_h), baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, font_scale, max(1, line_width - 1))
        text_top = max(0, y1 - text_h - baseline - 6)
        cv2.rectangle(canvas, (x1, text_top), (x1 + text_w + 6, text_top + text_h + baseline + 6), color, thickness=-1)
        cv2.putText(canvas, label, (x1 + 3, text_top + text_h + 1), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), max(1, line_width - 1), cv2.LINE_AA)
